...
@pytest.fixture()
def moneytranx_client(
    algorand_client: AlgorandClient, deployer: SigningAccount
) -> MoneytranxClient:
    factory = algorand_client.client.get_typed_app_factory(
        MoneytranxFactory, default_sender=deployer.address
    )
...
def test_says_hello(moneytranx_client: MoneytranxClient) -> None:
    result = moneytranx_client.send.hello(args=("World",))
    assert result.abi_return == "Hello, World"


def test_simulate_says_hello_with_correct_budget_consumed(
    moneytranx_client: MoneytranxClient,
) -> None:
    result = (
        moneytranx_client.new_group()
        .hello(args=("World",))
        .hello(args=("Jane",))
        .simulate()
    )
